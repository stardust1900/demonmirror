...
CALLBACK_URL = 'http://demonmirror.sinaapp.com/call_back'  # callback url
DM_UID = '5052773135'

try:
	REPAT = re.compile(u'[\U00010000-\U0010ffff]')
except re.error:
	REPAT = re.compile(u'[\uD800-\uDBFF][\uDC00-\uDFFF]')
            ...
            while mentions.total_number > 0:
                ...
                for st in mentions.statuses:
                    ...
                    if hasattr(st, 'retweeted_status') and hasattr(st.retweeted_status, 'pic_urls'):
                        text = REPAT.sub('',st.retweeted_status.text)
                        ...
                        if p:
                            continue
                        if hasattr(st.retweeted_status,'user'):
